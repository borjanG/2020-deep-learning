...
if __name__ == "__main__": 
    ...
    ax = plt.subplot(gs[:, 0])
    h = ax.imshow(Exact_sol, interpolation='nearest', cmap=cmocean.cm.thermal, 
    ...
    #ax.set_ylabel('$x$')
    ax.set_ylabel('space')
    ax.set_title('Finite difference solution', fontsize = 8)
    
    ########     Exact p(t,x,y)     ########### 
    ax = plt.subplot(gs[:, 1])
    h = ax.imshow(U_pred, interpolation='nearest', cmap=cmocean.cm.thermal, 
                  extent=[lb_sol[0], ub_sol[0], lb_sol[1], ub_sol[1]], 
                  origin='lower', aspect='auto')
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.05)
    ...
    #ax.set_ylabel('$x$')
    ax.set_ylabel('space')
    ax.set_title('Data-driven solution', fontsize = 8)
